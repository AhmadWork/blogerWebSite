...
while ($row = mysql_fetch_array($stmt)) {
		echo "<fieldset style='border-radius: 5px; width:98%; padding: 5px; min-height:20px; border:3px solid #1f497d;'><legend onclick='' style='background: #25587E; color: #fff; font-size: 14px; border-radius: 5px; box-shadow: 0 0 0 5px #ddd;'>" . $row['user_name'] . "<form action='blogger.php' method='post'><input type='hidden' name='myid' id=''value='".$row['user_id']."'/><button type='submit' style='width:100%; height:25px; float:left;'> <b>view bloger</b> </button></form></legend><br>";
		echo "".$row['post']."";
		if (isset($_SESSION['ID'])){
